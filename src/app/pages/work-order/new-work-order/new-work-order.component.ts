...
export class NewWorkOrderComponent implements OnInit, AfterViewInit {
  ...
  vendorsList() {
    this.apiCalls
      .get(this.endpoints.GET_VENDORS_LIST)
      .pipe(
        catchError(async (err) => {
          this.utils.showErrorDialog(this.dialog, err);
        })
  ...
  getAllJobs() {
    ...
    this.apiCalls
      .get(this.endpoints.LIST_JOBS, queryParam)
      .pipe(
        catchError(async (err) => {
          this.utils.showErrorDialog(this.dialog, err);
        })
  ...
  setJobDataOnUi(data: any) {
    ...
    setTimeout(() => {
      ...
      let site = this.siteList.filter(
        (obj) => obj.code == data.siteDetails?.code
      );
  ...
  submitWorkOrder(status: string) {
    ...
    if (workRateValue) {
      this.workOrderData.controls['workRate'].setValue(
    ...
    if (minBudget) {
      this.workOrderData.controls['minBudget'].setValue(
        minBudget.replace(/,/g, '')
      );
    }
    ...
    if (
      ...
      (workRateValue || (minBudget && maxBudget))
    ) {
      if (
      ...
      this.isLoading = true;
      this.workOrderData.controls['startDate'].setValue(
        this.changeDateToUtc(this.workOrderData.controls['startDate'].value)
      );
      this.workOrderData.controls['endDate'].setValue(
        this.changeDateToUtc(this.workOrderData.controls['endDate'].value)
      );
      ...
      this.apiCalls
        ...
        .subscribe((response) => {
          if (this.isLoading) {
            ...
            docData.append('workOrderId', response);
            this.apiCalls
              .post(this.endpoints.CREATE_WORK_ORDER_DOCUMENT, docData)
              .pipe(
                catchError(async (err) => {
                  this.isLoading = false;
